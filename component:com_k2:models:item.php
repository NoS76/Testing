	...
	function prepareFeedItem(&$item)
	{
		...
		if ($params->get('feedItemAttachments'))
		{
			...
			if (count($attachments))
			{
				...
				{
					$item->description .= '<li><a title="'.htmlentities($attachment->titleAttribute, ENT_QUOTES, 'UTF-8').'" onclick="openInAppBrowserBlank('.$attachment->link.')" href="'.$attachment->link.'">'.$attachment->title.'</a></li>';
				}
				$item->description .= '<ul></div>';
			}
		}
